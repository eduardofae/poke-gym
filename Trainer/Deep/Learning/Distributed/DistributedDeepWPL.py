...
class DistributedDeepWPL:
    class Trainer:
        ...
        def __init__(self, env, optimizer, g_net, t_net, e_rate, pi_l_rate, y, leader, name='player',
            ...
            self.net = DistributedDeepWPL.Network(optimizer, env, name='player_' + self.name, global_id=global_id)
        ...
        def compute_pi_q(self, s, a, r, t):
            ...
            pi_t, max_q = self.sess.run([self.t_net.policy, self.t_net.max_q], feed_dict=feed_dict)
            self.pi_t, max_q = pi_t[0], max_q[0]
            self.s_batch += [s]
            ...
            for _a in range(self.n_actions):
                ...
                for __a in range(self.n_actions):
                    delta += self.q_t[_a] - self.q_t[__a]
                    delta /= self.n_actions - 1
        ...
        def accumulate_grads_on_global(self):
            ...
            feed_dict = {self.net.target_q: self.q_t_batch, self.net.target_pi: self.pi_t_batch,
                         self.net.in_state: self.s_batch}
            ...
            with self.sess.graph.as_default():
                ...
                while ep < self.n_eps:
                    ...
                    while True:
                        ...
                        s0 = s1
                        s1, r, t, _ = self.env.step(tuple(a))
                        # for each player
                        for i, p in enumerate(self.player):
    ...
    def train(env, g_l_rate, concurrent_games, pi_l_rate, y, tau, n_eps, n_steps, e_rate, n_players, model_path,
        ...
        with tf.device(
            ...
            while p < n_players:
                g_net += [
                    {'global': DistributedDeepWPL.Network(optimizer, env, name='global_' + str(p), global_id=p),
                     'target': DistributedDeepWPL.Network(optimizer, env, name='target_' + str(p), global_id=p)}]
                print('global_' + str(p))
                print('target_' + str(p))
                p += 1
            game_pool = []
        ...
        with tf.compat.v1.train.MonitoredTrainingSession(master=server.target, is_chief=(task_index == 0),
            ...
            game_pool[task_index].set_session(sess)
            game_pool[task_index].run()
